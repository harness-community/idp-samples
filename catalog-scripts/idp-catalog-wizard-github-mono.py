...
metadata:
  ...
  name: {dirName}
  annotations:
    backstage.io/source-location: url:{dirPath}
spec:
  type: service
  system: {orgName}
...
def get_directories_in_repo(organization, repo_name, token, path):
    url = f"https://api.github.com/repos/{organization}/{repo_name}/contents/{path}"
    headers = {
...
def list_repositories(organization, token, path, repo_name, dir_pattern=None):

    yaml_files_created = 0
    print(f"Directories in {organization}:")

    directories = get_directories_in_repo(organization, repo_name, token, path)
...
def create_or_update_catalog_info(organization, dirName, dirPath):
    ...
    if os.path.exists(yaml_file_path):
        ...
        dirLocation = f"https://github.com/{organization}/{current_directory}"
        updated_content = yaml_content_template.format(dirName=dirName, dirPath=dirLocation, orgName=organization)
        with open(yaml_file_path, "w") as file:
            file.write(updated_content)
    else:
        # Create new YAML file
        with open(yaml_file_path, "w") as file:
            file.write(yaml_content_template.format(dirName=dirName, dirPath=dirPath, orgName=organization))
...
def register_yamls(organization, account, x_api_key):
    ...
    files = [name for name in os.listdir("./services") if os.path.isfile(os.path.join("./services", name))]
...
def main():
    ...
    if args.create_yamls:
        if args.org == None or args.token == None or args.repo_name == None or args.path == None:
            ...
            exit()
        list_repositories(args.org, args.token, args.path, args.repo_name)
